...
ax.scatter(X[y == -1, 0], X[y == -1, 1], X[y == -1, 2], color='blue', marker='x', label='Другие')

# Визуализация разделяющей плоскости (только финальная)
if abs(w[3]) > 1e-6:  # избегаем деления на ноль
    xx, yy = np.meshgrid(
...
ax.legend()
plt.title('Классификация с 3 признаками')
plt.show()